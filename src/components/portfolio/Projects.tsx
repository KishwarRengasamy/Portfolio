...
const projects: Project[] = [
  {
    ...
    pitch: "A wearable smart glove that continuously monitors hand tremor, heart rate, and SpO₂ using multi-sensor fusion on an ESP32. Cloud analytics correlate signals in real time to surface latent stress markers before they escalate.",
    pitchHighlights: ["smart glove", "multi-sensor fusion", "latent stress markers"],
    domain: "Embedded/IoT", year: "2025",
    stack: ["ESP32", "IoT", "MPU6050", "MAX30102", "Embedded C", "Cloud"],
    accent: "#FF5A36", visual: "neuro",
  },
  {
    title: "Hybrid Microplastic Detection",
    subtitle: "In oil and water, on-device.",
    pitch: "An affordable TinyML embedded system that detects microplastics in water and oil samples using an ESP32-CAM paired with a macro lens. On-device inference plus a Gen AI API layer make it a portable, scalable environmental monitoring tool.",
    pitchHighlights: ["TinyML", "On-device inference", "Gen AI API"],
    domain: "AI", year: "2025",
    stack: ["Python", "TinyML", "ESP32-CAM", "Macrolens", "Gen AI API"],
    accent: "#FF5A36", visual: "micro",
  },
  {
    title: "DORA",
    subtitle: "AI-powered smart travel assistant for transport analytics.",
    pitch: "A travel assistance platform that automatically captures trip data for transport research. GPS-based live tracking, offline alerts, multilingual support, AI-driven travel-mode prediction, and encrypted transmission feed cloud analytics that turn journeys into mobility insight.",
    pitchHighlights: ["GPS-based live tracking", "AI-driven travel-mode prediction", "mobility insight"],
    domain: "Full-Stack", year: "2025",
    stack: ["Flutter", "Dart", "React", "Python", "MongoDB", "Google Maps API"],
    accent: "#FF5A36", visual: "dora",
  },
  {
    title: "Hybrid Dual-Brush Solar Cleaning Robot",
    subtitle: "Water-free PV panel efficiency for utility-scale sites.",
    pitch: "An autonomous, water-free solar panel cleaning robot powered by a hybrid energy system combining solar charging and a rechargeable battery. Dual rotating brushes clear dust; IR edge sensors keep the rover safely on the array.",
    pitchHighlights: ["water-free", "Dual rotating brushes", "IR edge sensors"],
    domain: "Sustainability", year: "2025",
    stack: ["Arduino", "ESP32", "Embedded C", "IoT", "Solar", "IR Sensors"],
    accent: "#FF5A36", visual: "solar",
  },
  {
    title: "Nutri Vault",
    subtitle: "Smart onion storage system for post-harvest loss reduction.",
    pitch: "A low-cost smart storage system combining chitosan coating, biochar sachets, and IoT-based environmental monitoring to extend onion shelf life. Continuous temperature, humidity, CO₂, and ethylene tracking is exposed to farmers via a mobile interface.",
    pitchHighlights: ["chitosan coating", "post-harvest loss", "IoT-based environmental monitoring"],
    domain: "Sustainability", year: "2024",
    stack: ["Raspberry Pi", "IoT", "Python", "Sensors", "Chitosan", "Biochar"],
    accent: "#FF5A36", visual: "vault",
  },
];
...
function ScrambleText({ text, className }: { text: string; className?: string }) {
  ...
  useEffect(() => {
    ...
    if (!hovered) { setOutput(text); return; }
    let frame = 0;
    const TOTAL = 22;
    timerRef.current = setInterval(() => {
      ...
      frame++;
      if (frame > TOTAL) { clearInterval(timerRef.current!); setOutput(text); }
    }, 28);
    ...
    return (
      <svg viewBox="0 0 800 500" className="absolute inset-0 w-full h-full">
        ...
        <g stroke={accent} strokeWidth="1.2" fill="none">
          ...
          <motion.circle cx="600" cy="240" r="70" animate={{ r: [60, 75, 60], opacity: [0.6, 1, 0.6] }} transition={{ duration: 2.4, repeat: Infinity }} />
          ...
          <motion.circle cx="600" cy="240" r="6" fill={accent} animate={{ opacity: [1, 0, 1] }} transition={{ duration: 1.2, repeat: Infinity }} />
        </g>
      </svg>
    );
  }
  if (kind === "micro") {
    return (
      <svg viewBox="0 0 800 500" className="absolute inset-0 w-full h-full">
        <rect width="800" height="500" fill="#F1EFEA" />
        <g stroke={accent} strokeWidth="1.2" fill="none">
          <motion.circle cx="400" cy="250" r="180" animate={{ rotate: 360 }} transition={{ duration: 12, repeat: Infinity, ease: "linear" }} style={{ originX: "400px", originY: "250px" }} />
          <circle cx="400" cy="250" r="120" />
          <motion.circle cx="400" cy="250" r="60" animate={{ scale: [1, 1.08, 1] }} transition={{ duration: 2, repeat: Infinity }} style={{ originX: "400px", originY: "250px" }} />
          {Array.from({ length: 12 }).map((_, i) => {
            const a = (i / 12) * Math.PI * 2;
            return <motion.circle key={i} cx={400 + Math.cos(a) * 210} cy={250 + Math.sin(a) * 210} r="3" fill={accent} animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 2, repeat: Infinity, delay: i * 0.16 }} />;
          })}
    ...
    return (
      <svg viewBox="0 0 800 500" className="absolute inset-0 w-full h-full">
        ...
        <g stroke={accent} strokeWidth="1.2" fill="none">
          ...
          <motion.circle r="7" fill={accent} animate={{ cx: [40, 200, 380, 570, 760], cy: [400, 310, 320, 260, 200], opacity: [0, 1, 1, 1, 0] }} transition={{ duration: 3.5, repeat: Infinity, ease: "easeInOut", repeatDelay: 0.5 }} />
          ...
          <motion.circle cx="120" cy="55" r="4" fill={accent} animate={{ opacity: [1, 0, 1], r: [4, 8, 4] }} transition={{ duration: 1.5, repeat: Infinity }} />
        ...
        <g stroke={accent} strokeWidth="1.2" fill="none">
          {Array.from({ length: 6 }).map((_, i) => (
            <g key={i} transform={`translate(${100 + i * 100}, 120) rotate(-15)`}>
              ...
              {Array.from({ length: 6 }).map((__, j) => <line key={j} x1="0" y1={j * 40} x2="80" y2={j * 40} />)}
              <line x1="40" y1="0" x2="40" y2="240" />
            </g>
          ))}
          <motion.g style={{ originX: "640px", originY: "90px" }} animate={{ rotate: 360 }} transition={{ duration: 8, repeat: Infinity, ease: "linear" }}>
            {Array.from({ length: 8 }).map((_, i) => { const a = (i / 8) * Math.PI * 2; return <line key={i} x1={640 + Math.cos(a) * 48} y1={90 + Math.sin(a) * 48} x2={640 + Math.cos(a) * 76} y2={90 + Math.sin(a) * 76} strokeWidth="2" />; })}
          </motion.g>
          <circle cx="640" cy="90" r="42" />
          <circle cx="640" cy="90" r="70" opacity="0.6" />
          <motion.path d="M40 380 Q400 340 760 380" strokeWidth="1.5" animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 2.5, repeat: Infinity }} />
        </g>
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 800 500" className="absolute inset-0 w-full h-full">
      ...
      <g stroke={accent} strokeWidth="1.2" fill="none">
        ...
        <motion.circle cx="400" cy="250" r="70" animate={{ r: [65, 72, 65], opacity: [0.7, 1, 0.7] }} transition={{ duration: 2.4, repeat: Infinity }} />
        ...
        <motion.text x="240" y="460" fontFamily="monospace" fontSize="11" fill={accent} opacity="0.6" animate={{ x: [240, -100] }} transition={{ duration: 6, repeat: Infinity, ease: "linear" }}>
            ...
            >
              ...
              <div className="lg:col-span-5">
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  {p.stack.map((s, si) => <StackTag key={s} label={s} delay={si * 0.06} />)}
                </div>
